...
**dioxus-element-plug** is a Rust UI component library that brings Element Plus (Vue 3) design and CSS classes to the Dioxus 0.7 framework. It provides 115+ components styled with Element Plus `theme-chalk` CSS classes, rendered via Dioxus `#[component]` and `rsx!` macros.
...
dioxus-element-plug/
├── src/
│   ├── components/          # 115+ Element Plus style components (one .rs per component)
...
- Currently 29 tests, all passing
